...
RegisterCommand("panttivanki",function()
	takeHostage()
end)
...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Tarvitset aseen jolla uhkata panttivankia!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification("Ei pelaajia lähistöllä!")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Paina [G] luovuttaaksesi, [H] tappaaksesi")
